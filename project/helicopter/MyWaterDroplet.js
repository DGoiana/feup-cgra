...
export class MyWaterDroplet extends CGFobject {
...
export class MyWaterDropletSystem extends CGFobject {
    constructor(scene) {
        super(scene)
    ...
    update(deltaTime, currentTime, helicopterPosition, helicopterVelocity) {
        if (this.isDropping && currentTime - this.lastDropTime >= this.dropInterval) {
            this.createDroplet(helicopterPosition, helicopterVelocity)
            this.lastDropTime = currentTime
        }
        
        for (let i = this.droplets.length - 1; i >= 0; i--) {
            this.droplets[i].update(deltaTime)
            
            if (!this.droplets[i].isActive()) {
                this.droplets.splice(i, 1)
            }
        }
    }
    
    createDroplet(helicopterPosition, helicopterVelocity) {
        ...
        const droplet = new MyWaterDroplet(this.scene, size, position, velocity)
    ...
    checkFireCollisions(forest) {
        ...
        for (let droplet of this.droplets) {
            ...
            for (let i = 0; i < forest.fires.length; i++) {
                ...
                if (droplet.checkCollisionWithFire([fire[1], 0, fire[2]])) {
    ...
    getActiveDropletCount() {
        return this.droplets.filter(droplet => droplet.isActive()).length
    }
